const fs = require('fs')
const { readFile, writeFile, stat, mkdir, chmod, utimes, ...fsPromises } = fs.promises
 ...
 * @private
 **/
function outputFile(file, data, mode, created = null, updated = null) {
  return mkdir(dirname(file), { recursive: true })
    .then(() => writeFile(file, data))
    .then(() => (setFileTimes(file, created, updated)))
 ...
 *
 * @param {string} file
 * @param {string|void} created
 * @param {string|void} updated
 * @returns {Promise<void>}
 * @private
 */
function setFileTimes(file, created, updated) {
  if (created || updated) {
    const ctime = created ? new Date(created) : new Date();
    const mtime = updated ? new Date(updated) : new Date();
    return utimes(file, ctime, mtime);
  }
